    ...
    <>
      <div className="flex justify gap-3 p-2 bg-base-input outline-0 rounded border border-base-button appearance-none focus:border-yellow-dark">
        {data.tags.map((tag) => (
          <label
            key={tag.id}
            className="check-label disable-select flex justify-center px-3 py-2 cursor-pointer rounded-full text-yellow-dark font-Roboto text-[0.625rem] leading-3 uppercase font-bold"
          >
            <input
              {...register("checkbox")}
